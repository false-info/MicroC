head(custom) {
    ...
    fn print_error_message(I64 code) {
        if (code == 1) { os_write("unexpected token or invalid syntax") return 0 }
        if (code == 2) { os_write("custom syntax requires custom in head()") return 0 }
        if (code == 3) { os_write("inline x86 assembly requires an asm-x86-* mode in head()") return 0 }
        if (code == 4) { os_write("assignment to undeclared variable") return 0 }
        if (code == 5) { os_write("duplicate variable declaration") return 0 }
        if (code == 6) { os_write("identifier too long") return 0 }
        if (code == 7) { os_write("string literal too long") return 0 }
        if (code == 8) { os_write("wrong number of function arguments") return 0 }
        if (code == 9) { os_write("duplicate function declaration") return 0 }
        if (code == 10) { os_write("unterminated string literal") return 0 }
        if (code == 11) { os_write("compiler string pool exhausted") return 0 }
        if (code == 12) { os_write("hosted builtin is unavailable in raw .bin output") return 0 }
        if (code == 13) { os_write("kernel-only builtin cannot be used in an ELF program") return 0 }
        if (code == 14) { os_write("unknown function for fn_offset") return 0 }
        if (code == 15) { os_write("x86-16/x86-32 assembly requires raw .bin output") return 0 }
        if (code == 16) { os_write("custom x86-64 codegen cannot share x86-16/x86-32 head mode") return 0 }
        if (code == 17) { os_write("assembly mode is not enabled in head()") return 0 }
        if (code == 18) { os_write("invalid assembly register or operands") return 0 }
        if (code == 19) { os_write("unknown assembly label") return 0 }
        if (code == 20) { os_write("duplicate assembly label") return 0 }
        if (code == 21) { os_write("assembly value or branch is out of range") return 0 }
        if (code == 22) { os_write("unsafe operation is blocked in safe mode") return 0 }
        os_write("compilation failed")
        return 0
    }

    fn print_warning_message(I64 code) {
        if (code == 1) { os_write("custom enabled in head() but never used") return 0 }
        if (code == 2) { os_write("x86 assembly enabled in head() but never used") return 0 }
    ...
    fn driver_error(I64 code, I64 path) {
        ...
        if (code == 23) { os_write("could not open input file\n") return 1 }
        if (code == 24) { os_write("could not read input size\n") return 1 }
        if (code == 25) { os_write("source file is too large\n") return 1 }
        if (code == 26) { os_write("output arena guard was damaged\n") return 1 }
        if (code == 27) { os_write("output exceeded compiler arena\n") return 1 }
        if (code == 28) { os_write("could not write output file\n") return 1 }
        if (code == 29) { os_write("invalid command line\n") return 1 }

        os_write("compiler driver failure\n")
        return 1
    }
    ...
    fn compile_to_sink(I64 input_path, I64 output_name, I64 sink) {
        ...
        if (failed() != 0) {
            os_write("mcc: no output written\n")
            return 0 - 1
        }
    ...
    fn compiler_main(I64 count, I64 input_path, I64 option, I64 output_path) {
        if (count != 4) {
            os_write("MicroC AOT compiler\n")
            os_write("usage: mcc source.mc -o output\n")
            os_write("       mcc source.mc -o output.bin\n")
            return 1
        }
        ...
        return compile_aot(input_path, output_path)
    }
